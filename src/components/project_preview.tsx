...
export default function ProjectCard({ title, description, types, variant }: ProjectCardProps) {
    ...
    return (
        <div className="relative w-80 h-96 m-8">
            <div className={`absolute w-12 h-12 ${typePosUp} ${typeToColor[types[0]]} rounded-full z-10`} />

            {types[1] ? (
                <div className={`absolute w-12 h-12 ${typePosDown} ${typeToColor[types[1]]} rounded-full z-10`} />
            ) : (
                <div className={`absolute w-12 h-12 ${typePosDown} ${typeToColor[types[0]]} rounded-full z-10`} />
            )}
