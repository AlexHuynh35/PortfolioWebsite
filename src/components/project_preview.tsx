...
export default function ProjectCard({ title, description, types }: ProjectCardProps) {
    return (
        <div className="relative w-80 h-64 m-8">
            ...
            {types[1] ? (
                ...
                <div className={`absolute w-12 h-12 -bottom-2 -right-2 ${typeToColor[types[0]]} rounded-full z-10`} />
            )}
            ...
            {types[1] ? (
                ...
                <div className={`absolute inset-0 translate-x-2 translate-y-2 ${typeToColor[types[0]]} rounded-xl -z-20`} />
            )}
