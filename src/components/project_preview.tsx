...
export default function ProjectCard({ slug, title, date, types, collaborators, variant }: ProjectCardProps) {
    ...
    <div className="relative w-70 aspect-[5/6] hover:scale-105">
      <Link href={`/project/${slug}`}>
        ...
        <div className={`relative w-full h-full bg-slate-200 shadow-lg p-4 flex flex-col text-center justify-center ${clipPath}`}>
          ...
          <p className="text-xl text-gray-800 mt-2 font-rajdhani">{formatDate(date)}</p>
        </div>
      </Link>
    </div>
  );
